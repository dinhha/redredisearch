 ...
 *
 * @param {Function(err, total, res)} fn
 * @return {Query} for chaining
 * @api public
 */
...
Query.prototype.end = function (fn) {
  ...
  if (direct) {
    ...
    rediSearchQuery = rediSearchQuery.map(w => w + '*').join(joiner);
  }

  if (this._filteres.tags && this._filteres.tags.length > 0) {
    let tags = this._filteres.tags.map(tagsFilter => {
      if (tagsFilter.tags && tagsFilter.tags.length > 0) {
        let tags = tagsFilter.tags || [];
        tags = tags.map(t => t && t.trim().replace(/\s+/g, '\\ '))
          .filter(t => t != null)
          .join(' | ');

        let exclude = tagsFilter.exclude ? '-' : '';

        return `${exclude}@${tagsFilter.field}:{${tags}}`;
      }
    });
  ...
  if (this._filteres.fields) {
    for (const field in this._filteres.fields) {
      if (this._filteres.fields.hasOwnProperty(field)) {
        const qs = this._filteres.fields[field];
        if (qs instanceof Array) {
          rediSearchQuery += '(' + (qs || []).map(w => `(@${field}:${w})`).join('|') + ')';
        } else {
          rediSearchQuery += ` (@${field}:${qs})`;
        }
      }
    }
  }

  if (this._filteres.numeric.length > 0) {
    this._filteres.numeric.forEach(filter => {
      rediSearchQuery += ` @${filter.field}:[${filter.min} ${filter.max}]`;
    });
  }

  if (this._filteres.geo && this._filteres.geo.field) {
    const filter = this._filteres.geo;
    rediSearchQuery += ` @${filter.field}:[${filter.lng} ${filter.lat} ${filter.radius} ${filter.unit}]`;
  }
  ...
  if (this._sorts != null && Object.keys(this._sorts).length > 0) {
    for (const key in this._sorts) {
      args.push('SORTBY', key, this._sorts[key]);
    }
  }

  if (this._returns && this._returns.fields) {
    args.push('RETURN', this._returns.length, this._returns.fields.join(' '));
  } else {
    args.push('NOCONTENT');
  }

  if (this.keysFilter && this.keysFilter.keys) {
    args.push(`INKEYS`, `${this.keysFilter.length}`, ...this.keysFilter.keys);
  }
...
/**
 * @returns {Promise<{total: number, result: [string]}>}
 */
Query.prototype.endPromise = function () {
  return new Promise((resolve, reject) => {
    this.end((err, total, res) => {
      if (err) {
        return reject(err);
      } else {
        resolve({total, result: res});
      }
    })
  })
}
 ...
 * @param {string} field
 * @param {object} opts
 * @param {(number | '-inf')} opts.min
 * @param {(number | '+inf')} opts.max
 * 
 * @returns {Query}
 */
Query.prototype.numericFilter = function (field, { min = '-inf', max = '+inf' } = opts) {
  this._filteres.numeric.push({
    field,
    min,
    max
  });
 ...
 */
Query.prototype.tagsFilter = function (field, tags, {exclude = false} = {}) {
  if (typeof tags == 'string') {
    tags = [tags]
  }

  this._filteres.tags.push({
    field,
    tags,
    exclude: !!exclude
  });
 ...
 */
Query.prototype.fieldFilter = function (field, qs) {
  this._filteres.fields[field] = qs;
  return this;
}
...
/**
 * @param { Object.<string, (1 | -1)> } sort
 */
Query.prototype.sortBy = function (sort) {
  if (sort && Object.keys(sort).length > 0) {
    for (const key in sort) {
      if (sort.hasOwnProperty(key)) {
        const val = sort[key];
        this._sorts[key] = val == 1 ? 'ASC' : 'DESC';
      }
    }
  }
 ...
 * @returns {Query}
 */
Query.prototype.fieldsReturn = function (fields) {
...
Search.prototype.remove = function (id, opt = { deleteDocument: false }, fn) {
  fn = fn || noop;
  var key = this.key;
  var db = this.client;
  let args = [
